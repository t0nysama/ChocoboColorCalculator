...
Prefer a normal Windows program? Download the self-contained desktop edition from the [latest desktop release](https://github.com/t0nysama/ChocoboColorCalculator/releases/tag/desktop-v1.1.0).
...
- One enforced **closest-safe target** policy for every calculation. The engine expands its search when necessary, then mathematically proves that the selected lattice point is the closest unclamped endpoint to the published swatch with at least 3 RGB units of true color-boundary clearance. This removes the old arbitrary search-box fallback and preserves the successful 19-apple, 23-pear, 32-berry Desert Yellow to Soot Black recipe.
