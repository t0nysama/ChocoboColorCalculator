...
- Exact ordered feed list, total fruit counts, simulated RGB after every step, and predicted resulting named color. The in-game route includes a dedicated color-preview swatch for every feed, with its nearest named plumage color, RGB channels, and hex value on hover. Reliability is reported as the true Euclidean distance to the nearest named-color boundary, rather than a less meaningful difference between center distances.
